...
Download helm artifacts from [`github`](https://github.com/opensearch-project/helm-charts.git)
...
2. Edit helm values in helm-charts/charts/opensearch/values.yaml

3. Login to your Openshift Cluster

4. Service account are required, so value for rbac.create must be true.
 ```
rbac:
  create: true
  serviceAccountAnnotations: {}
  serviceAccount: "sa-opensearch"
  serviceAccountName: "sa-opensearch"
 ```
5. Execute helm install from directory.
...
6. After installed apply security constraint for service account create with the deployment

 ```
 $ oc adm policy add-scc-to-user privileged -z sa-opensearch

 $ oc adm policy add-scc-to-user anyuid -z sa-opensearch
 ```
